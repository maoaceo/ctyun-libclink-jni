...
4. **现代化二次元/暗色毛玻璃 Web 控制面板**：
   ...
   - 📱 **手机扫码授权登录**：未登录时网页展示官方最新二维码，扫码成功后自动隐藏并展示授权就绪卡片；
   - 🖥️ **设备编码直接识别**：无需寻找数字 ID，直接粘贴输入天翼云电脑编码（如 `D0026090823728443`）或点击“自动探测”即可加入保活；
...
./setup.sh
```
> `setup.sh` 已经过全面优化，智能兼容 Ubuntu 24.04/22.04/20.04（自动适配 `t64` 音频库）及 Debian 10/11/12，增量补齐 XCB 与虚拟屏幕依赖，并带有完整性自检。
...
为了保证服务器重启后自动恢复挂机保活，可直接注册为系统守护服务：
...
4. **添加云电脑**：
   - **方式一**：在输入框直接粘贴你的设备编码（例如 `D0026090823728443`），点击“添加到轮询队列”；
   - **方式二**：点击“🔍 自动探测”，系统将自动检索账号名下的所有云电脑并一键载入；
